...
$pixel_url = "http://pixelquestgames-sahaj-totla.us-west-1.elasticbeanstalk.com/users.php"; // Replace with the correct URL to users.php
$genai_url = "http://genaiwear.us-west-1.elasticbeanstalk.com/users.php";
$servic_url = "http://ecommerce.us-west-1.elasticbeanstalk.com/users.php";
...
if (is_array($users)) {
    echo "<h1>GenAIwear User List</h1>";
...
// Check if the response is an array of users
if (is_array($users)) {
    echo "<h1>Pixel Quest Games User List</h1>";
    foreach ($users as $user) {
        echo "User: " . htmlspecialchars($user['first_name']) . " " . htmlspecialchars($user['last_name']) . 
             "<br>Email: " . htmlspecialchars($user['email']) . 
             "<br>Home Phone: " . htmlspecialchars($user['home_phone']) . 
